...
with tempfile.TemporaryDirectory(prefix='mori-http-') as tmp:
    ...
    try:
        for download_mode,preview_mode in [('proxy','proxy'),('presigned','proxy'),('proxy','presigned'),('presigned','presigned')]:
            ...
            env.update(BROWSER_LISTEN_ADDR=f'127.0.0.1:{port}',BROWSER_USERNAME='tester',BROWSER_PASSWORD='test-browser-password',BROWSER_PUBLIC='false',BROWSER_DOWNLOAD_MODE=download_mode,BROWSER_PREVIEW_MODE=preview_mode,S3_ENDPOINT=f'http://127.0.0.1:{origin.server_port}',S3_REGION='ap-northeast-2',S3_BUCKET='test-bucket',S3_PREFIX='public/',S3_FORCE_PATH_STYLE='true',S3_ACCESS_KEY_ID='TESTACCESS',S3_SECRET_ACCESS_KEY='test-secret-key')
            process=subprocess.Popen([str(binary)],cwd=tmp,env=env,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE)
            try:
                for _ in range(100):
                    try:
                        if request(base+'/_mori/healthz')[0]==200: break
                    except OSError: time.sleep(.05)
                else: raise RuntimeError('mori failed to start')
                headers={'Authorization':AUTH}
                assert request(base+'/_mori/api/config')[0]==401
                status,_,body=request(base+'/_mori/api/config',headers=headers)
                ...
                status,_,body=request(base+'/_mori/api/list',headers=headers)
                listing=json.loads(body)
                assert status==200 and len(listing['entries'])==2 and listing['cursor']
                before=len(fixture.EVENTS)
                status,h,again=request(base+'/_mori/api/list',headers=headers)
                ...
                for download in (False,True):
                    ...
                    url=base+'/_mori/api/object?key=README.md'+('&download=1' if download else '')
                    ...
                    if mode=='proxy': assert h['X-Cache'] in ('MISS','HIT')
                ...
                before=len(fixture.EVENTS)
                status,h,body=request(base+'/_mori/api/preview?key=README.md',headers=headers)
                ...
                for suffix in ('', '&download=1'):
                    status,h,body=request(base+'/_mori/api/object?key=README.md'+suffix, method='HEAD', headers=headers)
                ...
                status,_,reply=request(base+'/_mori/api/archive',method='POST',body=body,headers={**headers,'Content-Type':'application/json','X-Mori-Request':'1','Origin':base})
                ...
                before=len(fixture.EVENTS)
                status,_,reply=request(base+'/_mori/api/archive',method='POST',body=json.dumps({'prefix':'','keys':['documents/','README.md','documents/']}).encode(),headers={**headers,'Content-Type':'application/json','X-Mori-Request':'1','Origin':base})
                assert status==200,reply
                ticket=json.loads(reply)
                ...
                print(f'PASS embedded-cache compiled HTTP integration: {download_mode=} {preview_mode=}, signed URL follow, server-side HEAD/list, recursive paginated ZIP paths/empty folders/Store/CRC/single-use')
            finally:
                process.terminate()
