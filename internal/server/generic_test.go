...
func (m *memBackend) List(ctx context.Context, prefix, cursor string) (backend.Listing, error) {
	...
	for _, f := range files {
		l.Entries = append(l.Entries, backend.Entry{Key: f, Name: strings.TrimPrefix(f, prefix), Size: int64(len(m.files[f])), ETag: backend.VersionTag(int64(len(m.files[f])), m.modified), Type: "text/plain"})
	}
	return l, nil
}
...
func (m *memBackend) Stat(ctx context.Context, key string) (backend.Object, error) {
	...
	if body, ok := m.files[key]; ok {
		return backend.Object{Key: key, Size: int64(len(body)), ETag: backend.VersionTag(int64(len(body)), m.modified), Modified: m.modified}, nil
	}
	if m.isDir(key) {
...
func TestGenericObjectRangeHEADAndConditional(t *testing.T) {
	...
	if w = call(a, "GET", "/api/object?key=missing.txt", nil); w.Code != 404 {
		t.Fatal(w.Code)
	}
	if w = call(a, "GET", "/api/object?key=docs", nil); w.Code != 404 {
		t.Fatal("directory served as file", w.Code)
	}
	if w = call(a, "GET", "/api/object?key=docs/sub/zero.txt", nil); w.Code != 200 || w.Body.Len() != 0 || w.Header().Get("Content-Length") != "0" {
...
func TestGenericListPreviewAndPresignedFallback(t *testing.T) {
	...
	w := call(a, "GET", "/api/object?key=README.md&download=1", nil)
	...
	w = call(a, "GET", "/api/preview?key=README.md", nil)
	var preview map[string]any
	json.Unmarshal(w.Body.Bytes(), &preview)
	if w.Code != 200 || preview["mode"] != "proxy" || preview["url"] != "/api/object?key=README.md" || preview["kind"] != "text" {
		t.Fatal(w.Code, preview)
	}
	for _, want := range []string{"MISS", "HIT"} {
		w = call(a, "GET", "/api/list?prefix=docs/", nil)
	...
	if w = call(a, "GET", "/api/list?prefix=nope/", nil); w.Code != 404 {
		t.Fatal(w.Code)
	}
}
