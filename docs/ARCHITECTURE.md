...
|---|---|---|---|
| `/`, `/폴더/` | 서버가 렌더링한 파일 목록 | 원본 인덱스 | 원본 인덱스 |
...
browser 모드의 목록은 서버가 `html/template`로 렌더링한다. 브라우저 UI는 그 DOM에 선택·ZIP·미리보기·정렬을 붙일 뿐 행을 만들지 않고, 다음 페이지도 같은 서버 렌더링 결과를 받아 이어 붙인다. 목록을 그리는 코드가 한 곳뿐이라 스크립트가 있는 화면과 없는 화면이 갈라지지 않는다. 아이콘은 문서에 인라인된 SVG 스프라이트를 `<use>`로 참조하므로 경로 데이터도 한 벌이다. 정렬과 페이지 이동은 링크(`?sort=`, `?cursor=`)여서 스크립트 없이도 동작한다.

한 객체는 한 URL을 가진다. 브라우저 UI의 파일 링크·미리보기 descriptor·다운로드 버튼이 모두 객체의 경로를 사용하므로, 브라우저 캐시와 mori 앞단의 캐시에서 항목이 갈라지지 않는다. UI가 만드는 링크와 서버가 만드는 URL은 같은 규칙(`encodeURIComponent`)으로 인코딩하여 글자 단위로 일치시킨다. object API는 예약 경로와 충돌하는 정확한 key를 읽는 통로로 남는다.
