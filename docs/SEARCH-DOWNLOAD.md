> 목록/ZIP 전달 구조를 설명합니다. 내장 미리보기·CORS는 [PREVIEW.md](PREVIEW.md)를 참고하세요.

# 화면 목록과 ZIP 재귀 조회의 구분
...
presign은 **GetObject(GET)만** 허용합니다. HEAD는 객체 서버 경로로, 목록은 서버의 ListObjectsV2로 처리합니다. 개별 다운로드/미리보기 GET은 기존 ENV에서 독립적으로 proxy/presigned를 선택합니다. ZIP 자체는 언제나 서버를 경유합니다.
